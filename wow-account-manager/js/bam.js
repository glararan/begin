...
var accountBalance = {
    ...
    refreshBalance: function() {
		...
		$.ajax({
			timeout: 60000,
            url: Core.staticUrl + '/data/refresh-balance.html',
			success: function(msg) {
                if (msg !== '') {
                    var currencies = msg;
	...
	setAccountBalanceCurrency: function(selected) {
			...
			$.ajax({
				timeout: 60000,
            url: Core.staticUrl + '/management/set-account-cookie.html?prefix=CUR&value=' + accountBalance.accountBalanceCurrency + '&_=' + new Date().getTime(),
				success: function(msg) {

...
}(function ($)
{
	$.fn.tweet = function(o)
	{
		...
		var url_regexp = /\b((?:[a-z][\w-]+:(?:\/{1,3}|[a-z0-9%])|www\d{0,3}[.]|[a-z0-9.\-]+[.][a-z]{2,4}\/)(?:[^\s()<>]+|\(([^\s()<>]+|(\([^\s()<>]+\)))*\))+(?:\(([^\s()<>]+|(\([^\s()<>]+\)))*\)|[^\s`!()\[\]{};:'".,<>?Â«Â»â€śâ€ťâ€â€™]))/gi;
		...
		function relative_time(date)
		{
			...
			if (delta < 1)
				...
				r = delta + ' sekund(y) zpět';
			else if(delta < 120)
				r = 'před minutou';
			else if(delta < (45*60))
				r = 'před ' + (parseInt(delta / 60, 10)).toString() + ' minuty';
